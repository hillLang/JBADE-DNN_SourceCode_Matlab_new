...
opts.sgd_max_epoch = 100; % maximum number of training epochs
...
opts.hid_struct = [256 256 256 256]; % num of hid layers and units
...
train_target(:, 1) = (train_target(:, 1)+90)/180;
test_label(:, 1) = (test_label(:, 1)+90)/180;
cv_label(:, 1) = (cv_label(:, 1)+90)/180;
train_target(:, 2) = train_target(:, 2)/3;
test_label(:, 2) = test_label(:, 2)/3;
cv_label(:, 2) = cv_label(:, 2)/3;
